...
chat = ChatLiteLLM(model="command-nightly", streaming=True,
    verbose=True,
...
tools = [
    Tool(
        ...
        func=qa_chain.run,
        description="Useful for summarising documents",
    ),
    Tool(
    name="python_repl",
...
if user_query := st.chat_input(placeholder="What is this article about?"):
    ...
    with st.chat_message("assistant"):
        ...
        response = agent.run({"query": user_query}, callbacks=[retrieval_handler])
        st.write(response)